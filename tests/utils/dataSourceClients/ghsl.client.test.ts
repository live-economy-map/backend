...
describe.skip('ghsl.client', () => {
  ...
  it.todo(
    'handles a malformed/incomplete provider response distinguishably from an empty result — ' +
      "OPEN ITEM: blocked on Doc 8-6's undecided Earth Engine call shape (same as viirs.client.test.ts). " +
      "Once ghsl.client.ts's real EE call is implemented, mock that specific call to resolve an " +
      'unexpected shape and assert the result either rejects.toThrow() or resolves a value ' +
